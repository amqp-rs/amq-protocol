...
pub fn gen_raw_value<'a, W: Write + BackToTheBuffer + 'a>(v: &'a AMQPValue) -> impl SerializeFn<W> + 'a {
    move |x| match *v {
...
pub fn gen_short_string<'a, W: Write + 'a>(s: &'a str) -> impl SerializeFn<W> + 'a {
    let len = s.len() as ShortShortUInt;
    let bytes = s.as_bytes();
    tuple((gen_short_short_uint(len), slice(bytes)))
}
...
pub fn gen_long_string<'a, W: Write + 'a>(s: &'a str) -> impl SerializeFn<W> + 'a {
    let len = s.len() as LongUInt;
    let bytes = s.as_bytes();
    tuple((gen_long_uint(len), slice(bytes)))
}

/// Generate the [FieldArray](../type.FieldArray.html) in the given buffer (x)
pub fn gen_field_array<'a, W: Write + BackToTheBuffer + 'a>(a: &'a FieldArray) -> impl SerializeFn<W> + 'a {
...
pub fn gen_timestamp<'a, W: Write>(t: Timestamp) -> impl SerializeFn<W> {
    gen_long_long_uint(t)
}

/// Generate the [FieldTable](../type.FieldTable.html) in the given buffer (x)
pub fn gen_field_table<'a, W: Write + BackToTheBuffer + 'a>(t: &'a FieldTable) -> impl SerializeFn<W> + 'a {
    gen_with_len(many_ref(t, move |entry| gen_field_entry(entry)))
}
...
pub fn gen_flags<'a, W: Write + 'a>(f: &'a AMQPFlags) -> impl SerializeFn<W> + 'a {
    move |x| f.get_bytes().iter().fold(Ok(x), |acc: GenResult<W>, b| {
...
mod test {
    ...
    fn test_gen_short_string() {
        assert_eq!(test_gen!(&mut [0][..],             gen_short_string, ""),     Ok((vec![0],                     1)));
        assert_eq!(test_gen!(&mut [0, 0, 0, 0, 0][..], gen_short_string, "test"), Ok((vec![4, 116, 101, 115, 116], 5)));
    }

    #[test]
    fn test_gen_long_string() {
        assert_eq!(test_gen!(&mut [0, 0, 0, 0][..],             gen_long_string, ""),     Ok((vec![0, 0, 0, 0],                     4)));
        assert_eq!(test_gen!(&mut [0, 0, 0, 0, 0, 0, 0, 0][..], gen_long_string, "test"), Ok((vec![0, 0, 0, 4, 116, 101, 115, 116], 8)));
    }
